  ...
  return (
    <div className='rounded-md border border-slate-500 px-4 py-4'>
      <div>
        ...
        {/* input should only be displayed while in edit mode (editCommentMode state has the comment's id as its value) */}
      ...
      {/* only admin or same user can edit or delete comments */}
      {props.user?.id === props.authorId || props.user?.isAdmin ? (
          ...
          props.editCommentMode === false ? (
            ...
            <>
              <button
                className='flex items-center gap-2 rounded-lg bg-black px-2 py-1.5 text-sm text-white hover:bg-gray-800'
                onClick={() =>
                  props.updateComment(props.postId, props.commentId)
                }
              >
                <MdCheckCircle />
                <span>Update</span>
